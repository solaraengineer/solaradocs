...
from .models import Project, Contributor, Pending, User, Backup, Audit, Documents, Teams, TeamMember, Changelog, \
    PendingAction, ViewerDocumentAccess, InviteCode, PromoCodes, InvoicePayment, TIER_LIMITS
...
def _record_invoice_payment(invoice, status, user=None, tier=None, paid_at=None):
    invoice_id = invoice.get('id')
    if not invoice_id:
        return None
    amount = invoice.get('amount_paid') or invoice.get('amount_due') or 0
    pdf_url = invoice.get('hosted_invoice_url') or invoice.get('invoice_pdf') or ''
    # URLField validators don't fire on update_or_create — enforce https scheme
    # here so a forged/malformed webhook can't sneak in a `javascript:` URL.
    if pdf_url and not pdf_url.startswith('https://'):
        pdf_url = ''
    email = (user.email if user else None) or invoice.get('customer_email') or ''
    username = user.username if user else ''
    resolved_tier = tier or (user.Tier if user else None) or 'personal'
...
def delete_project(request):
    ...
    try:
        ...
        if not isinstance(confirm_name, str) or confirm_name != project.project_name:
            return JsonResponse(
                {'success': False, 'error': 'Type the exact project name to confirm.'},
                status=400,
            )
...
@ratelimit(key='ip', rate='30/m', block=True)
def home(request):
    if request.user.is_authenticated and getattr(request.user, 'redirect_to_dashboard', True):
        return redirect('dashboard')
    context = {}
...
def stripe_webhook(request):
    ...
    if event['type'] == 'customer.subscription.created':
        ...
        if customer_id:
            ...
            if resolved_user and len(active_subs.data) > 1:
                logger.critical(
                    f"DUPLICATE SUBS: User {resolved_user.id} has {len(active_subs.data)} "
                    f"active subs on customer {customer_id}. "
                    f"Sub IDs: {[s.id for s in active_subs.data]}"
                )

        paid_at_ts = (invoice.get('status_transitions') or {}).get('paid_at') or invoice.get('created') or event.get('created')
        paid_at = datetime.fromtimestamp(paid_at_ts, tz=timezone.utc) if paid_at_ts else None
        _record_invoice_payment(invoice, status='paid', user=resolved_user, tier=tier, paid_at=paid_at)
        ...
        if customer_id:
            try:
                ...
                if user.Tier == 'free':
                    _record_invoice_payment(invoice, status='failed', user=user)
                    return HttpResponse(status=200)
        ...
        if sub_id:
            try:
                ...
                if not tier:
                    items = sub.get('items', {}).get('data', [])
                    if items:
                        tier = _tier_from_price_id(items[0].get('price', {}).get('id'))
            except stripe.error.StripeError:
                pass

        _record_invoice_payment(invoice, status='failed', user=resolved_user, tier=tier)
        ...
        if not invoice_id:
            logger.info(
                "charge.refunded for charge %s has no invoice; skipping",
                charge.get('id'),
            )
            return HttpResponse(status=200)
        updated = InvoicePayment.objects.filter(stripe_invoice_id=invoice_id).update(status='refunded')
        if updated == 0:
            logger.info(
                "charge.refunded for invoice %s matched no InvoicePayment row",
                invoice_id,
            )
...
@require_GET
def profile(request):
    # Fetch one extra row so we can detect "has more" without a second COUNT query.
    invoices = list(
        InvoicePayment.objects.filter(username=request.user.username)
        .order_by('-created_at')[:6]
    )
    has_more = len(invoices) > 5
    return render(request, 'profile.html', {
        'user': request.user,
        'invoices': invoices[:5],
        'has_more_invoices': has_more,
    })
...
def billing_history(request):
    invoices = InvoicePayment.objects.filter(
        username=request.user.username
    ).order_by('-created_at')
    return render(request, 'billing_history.html', {
        'user': request.user,
        'invoices': invoices,
    })
...
def billing_portal_session(request):
    ...
    try:
        ...
        logger.exception("billing_portal_session: stripe error for user %s", request.user.id)
        return redirect('profile')
...
@ajax_login_required
@ratelimit(key='ip', rate='30/m', block=True)
def toggle_redirect_to_dashboard(request):
    try:
        data = json.loads(request.body)
    except json.JSONDecodeError:
        return JsonResponse({'success': False, 'error': 'Invalid JSON'}, status=400)
    ...
    if not isinstance(value, bool):
        return JsonResponse({'success': False, 'error': 'enabled must be boolean'}, status=400)
    request.user.redirect_to_dashboard = value
    request.user.save(update_fields=['redirect_to_dashboard'])
    return JsonResponse({'success': True, 'enabled': value})
...
def recent_projects(request):
    """Used by the Cmd+K command palette to surface the user's recent projects."""
    qs = Project.objects.filter(owner=request.user).order_by('-created_at')[:5]
    return JsonResponse({
        'success': True,
        'projects': [
            {'id': p.id, 'name': p.project_name, 'url': f'/project/{p.id}/'}
            for p in qs
        ],
    })
