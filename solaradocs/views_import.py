...
def list_google_docs_view(request, project_id):
    try:
        project, err = _get_project_or_error(project_id)
        if err:
            return err
        ...
        docs = list_user_google_docs(request.user)
        if docs is None:
            return JsonResponse({
                'success': False,
                'error': 'Failed to fetch Google Docs. Please re-authenticate.',
        ...
        logger.exception("list_google_docs_view failed")
        return JsonResponse({'success': False, 'error': 'Something went wrong'}, status=500)
...
def start_google_import(request, project_id):
    ...
    try:
        ...
        if max_docs is not None:
            ...
            if len(doc_ids) > available_slots:
        ...
        for did in doc_ids:
            ...
            job_entries.append({'id': did, 'title': title})

        set_import_job(project_id, job_entries)

        import_google_docs_task.delay(
            request.user.id, project_id, team_id, doc_ids
        )
