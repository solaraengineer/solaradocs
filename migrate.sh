...
#
# Container roster (13, deterministic IP allocation in a fresh VPC):
...
#   3.  solaradocs-celery-soldocs-1     .4   solaradocs-celery:latest (worker)
#   4.  solaradocs-beat-soldocs-1       .5   solaradocs-beat:latest   (beat)
#   5.  otel-collector-soldocs-1        .6   otel-collector:latest (retagged)
#   6.  tempo-soldocs-1                 .7   grafana/tempo:2.7.0
#   7.  prometheus-soldocs-1            .8   prom/prometheus:latest
#   8.  grafana-soldocs-1               .9   grafana/grafana:latest
#   9.  alertmanager-soldocs-1         .10   prom/alertmanager:v0.27.0
#  10.  node-exporter-soldocs-1        .11   prom/node-exporter:latest
#  11.  redis-exporter-soldocs-1       .12   redis-exporter:latest (retagged)
#  12.  loki-soldocs-1                 .13   grafana/loki:3.0.0
#  13.  promtail-soldocs-1             .14   grafana/promtail:3.0.0
#
...
# sh -c so $REDIS_PASSWORD is expanded by the container's shell at start,
# not pre-baked into state.toml.
...
CELERY_IP="172.100.$OCTET.4"
BEAT_IP="172.100.$OCTET.5"
OTEL_IP="172.100.$OCTET.6"
TEMPO_IP="172.100.$OCTET.7"
PROMETHEUS_IP="172.100.$OCTET.8"
GRAFANA_IP="172.100.$OCTET.9"
ALERTMANAGER_IP="172.100.$OCTET.10"
NODE_EXPORTER_IP="172.100.$OCTET.11"
REDIS_EXPORTER_IP="172.100.$OCTET.12"
LOKI_IP="172.100.$OCTET.13"
PROMTAIL_IP="172.100.$OCTET.14"
log "subnet=$SUBNET → redis=$REDIS_IP web=$WEB_IP prom=$PROMETHEUS_IP graf=$GRAFANA_IP"
...
log "pulling stock observability images"
rsh "sudo docker pull redis:7-alpine \
  && sudo docker pull $OTEL_IMAGE_SRC \
  && sudo docker pull grafana/tempo:2.7.0 \
  && sudo docker pull prom/prometheus:latest \
  && sudo docker pull grafana/grafana:latest \
  && sudo docker pull prom/alertmanager:v0.27.0 \
  && sudo docker pull prom/node-exporter:latest \
  && sudo docker pull $REDIS_EXPORTER_SRC \
  && sudo docker pull grafana/loki:3.0.0 \
  && sudo docker pull grafana/promtail:3.0.0"
...
# alert-rules.yml is reused as-is from the repo root (no IPs in it).
cp "$SOLARADOCS_DIR/alert-rules.yml" "$STAGE/prometheus/alert-rules.yml"
...
rsh "sudo cp /tmp/soldocs-cfg/prometheus/prometheus.yml          /var/sol-ctl/volumes/$VPC_NAME/prometheus/config/ \
  ...
  && sudo cp /tmp/soldocs-cfg/otel-collector/otel-collector-config.yaml /var/sol-ctl/volumes/$VPC_NAME/otel-collector/config/ \
  && sudo cp /tmp/soldocs-cfg/tempo/tempo.yaml                    /var/sol-ctl/volumes/$VPC_NAME/tempo/config/ \
  && sudo cp /tmp/soldocs-cfg/loki/loki-config.yaml               /var/sol-ctl/volumes/$VPC_NAME/loki/config/ \
  && sudo cp /tmp/soldocs-cfg/promtail/promtail-config.yaml       /var/sol-ctl/volumes/$VPC_NAME/promtail/config/ \
...
# -------- 8. assign all 13 containers (deterministic IP order) --------------
...
log "assigning solaradocs-web ($WEB_IP, gunicorn)"
rsh "sudo solctl assign $WEB_IMAGE $VPC_NAME --cmd $(printf %q "$GUNICORN_CMD")"

log "assigning solaradocs-celery ($CELERY_IP, worker)"
...
rsh "sudo solctl assign $OTEL_IMAGE $VPC_NAME \
        --volume /var/sol-ctl/volumes/$VPC_NAME/otel-collector/config:/etc/otelcol-contrib \
        --cmd '--config /etc/otelcol-contrib/otel-collector-config.yaml'"

log "assigning tempo ($TEMPO_IP)"
rsh "sudo solctl assign grafana/tempo:2.7.0 $VPC_NAME \
...
rsh "sudo solctl assign prom/prometheus:latest $VPC_NAME \
...
rsh "sudo solctl assign prom/alertmanager:v0.27.0 $VPC_NAME \
...
# Redis password comes from REDIS_PASSWORD baked into the container env via .env.
rsh "sudo solctl assign $REDIS_EXPORTER_IMAGE $VPC_NAME \
        --cmd $(printf %q "redis_exporter --redis.addr=redis://$REDIS_IP:6379")"

log "assigning loki ($LOKI_IP)"
rsh "sudo solctl assign grafana/loki:3.0.0 $VPC_NAME \
...
rsh "sudo solctl assign grafana/promtail:3.0.0 $VPC_NAME \
...
check "django"        "http://$WEB_IP:8000/"                          "200"
